...
try:
    ...
    if current_stage == 1:
        if not state.get("step_completed", {}).get("stage_1_clicked", False):
            button = page.locator('text="Let\'s Draw!"')
            button.wait_for(state="visible", timeout=5000)
            button.click()
            state["step_completed"]["stage_1_clicked"] = True
            state["stage_1_done"] = True
    if current_stage == 2 or (current_stage == 1 and state.get("step_completed", {}).get("stage_1_clicked", False)):
        if not state.get("step_completed", {}).get("prompt_fetched", False):
