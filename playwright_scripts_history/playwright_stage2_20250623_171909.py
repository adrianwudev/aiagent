...
def main():
    ...
    try:
        ...
        print(f"DEBUG: type(state) = {type(state)}")

        try:
            if not isinstance(state, DictType):
                raise TypeError("state is not a dict (isinstance check failed)")
        except TypeError:
            if type(state) is not DictType:
                err_trace = traceback.format_exc()
                print(f"Type check failed for state even with fallback: {err_trace}")
                state = {}
                error_log_list = state.get("type_check_errors", [])
                if not isinstance(error_log_list, list):
                    error_log_list = []
                error_log_list.append({
                    "error": "state is not a dict after fallback check",
                    "traceback": err_trace,
