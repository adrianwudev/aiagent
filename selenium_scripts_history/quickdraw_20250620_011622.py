...
# --- Script Summary ---
# Purpose: Dispatch JavaScript mouse click events on buttons after removing blocking overlay 'newround-card' to start Quick, Draw! game.
# Result: Overcomes click interception by overlays using synthetic mouse event dispatch.
# Achieved: Robust clickable interaction for game start buttons.
# Score: 10/10
# Suggestions: Implement drawing gesture automation as next step.
# ----------------------
...
def dispatch_mouse_click(driver, btn):
    click_script = """
    ...
    try:
        ...
        print("Dispatched mouse click event via JavaScript.")
        return True
    except Exception as e:
        print(f"JavaScript mouse event dispatch failed: {e}")
        return False
...
def click_button_safe(driver, btn):
    if dispatch_mouse_click(driver, btn):
        return True
    try:
        btn.click()
